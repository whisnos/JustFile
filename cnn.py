...
if is_train:
    ...
    # picture = tf.reshape(picture, [-1, 28, 28, 1])
    #
    # # 载入模型
    ...
    #
    # 进行预测
    test_output = sess.run(logits, {input_x: picture})  # test_x[:20]
    inferenced_y = np.argmax(test_output, 1)
    # val_loss, val_acc = sess.run([loss, logits], feed_dict={input_x: mnist.test.images, output_y: mnist.test.labels})
    # print('val_loss val_acc',(val_loss, val_acc))
    # predict_result = sess.run(logits, feed_dict={input_x: picture,})
    # inferenced_y = np.argmax(predict_result, 1)
    print(inferenced_y, 'Inferenced numbers')  # 推测的数字
    print(np.argmax(test_y[16:50], 1), 'Real numbers')  # 真实的数字
    print("你导入的图片是：", inferenced_y[0])
# cv2.waitKey(150)
